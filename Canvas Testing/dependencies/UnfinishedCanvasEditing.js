...
class PaintCanvas {
    ...
    /* Expose for reuse by other scripts / later integration. */
    if (typeof window !== 'undefined') window.PaintCanvas = PaintCanvas;
    
    /* ═══════════════════════════════════════════════════════════
    DEMO INIT — wires the class to the page's controls.
    Delete this block (and keep `window.PaintCanvas`) if you only
    want the class, or replace it with your own integration.
    ═══════════════════════════════════════════════════════════ */
function initPaintCanvasDemo() {
    const $ = id => document.getElementById(id);
    const canvas = $('paint');
    if (!canvas) return;                       // not the demo page — skip init
    
    const pc = new PaintCanvas(canvas, { width: 480, height: 640, brushSize: 20, regionCount: 3 });
    window.pc = pc;                            // handy for console poking
    
    const log = msg => { const el = $('log'); if (el) el.textContent = msg; };
    
    // Stand-in for "brightness from constants": region idx -> 0..1.
    ...
    bind('regionCount', 'input', e => { pc.setRegionCount(parseInt(e.target.value, 10) || 1); rebuildRegionSelect(); });
    bind('regionSelect', 'change', e => pc.setActiveRegion(e.target.value));
    bind('brushSize', 'input', e => pc.setBrushSize(e.target.value));
    bind('paintMode', 'change', e => pc.setMode(e.target.value));
    bind('clearRegion', 'click', () => pc.clearRegion(pc.activeRegion));
    bind('clearAll', 'click', () => pc.clearAll());
    
    bind('tintToggle', 'click', () => {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initPaintCanvasDemo);
    } else {
    initPaintCanvasDemo();
    }